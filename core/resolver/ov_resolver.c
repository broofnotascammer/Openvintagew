...
ov_status_t ov_resolver_evaluate_pipeline(
    const ov_pipeline_request_t *pipeline_req,
    ov_pipeline_decision_t      *out_decision
) {
    if (!pipeline_req || !out_decision) return OV_ERROR_INVALID_PARAM;
    memset(out_decision, 0, sizeof(ov_pipeline_decision_t));

    const ov_hardware_profile_t *hw = pipeline_req->hardware;
    if (!hw) {
        hw = ov_hardware_get_active_profile();
    }

    out_decision->is_real_hardware = (!hw->is_simulated && hw->source == OV_HW_SOURCE_NATIVE);

    /* 1. Evaluate Compatibility */
    ov_os_compat_result_t os_eval;
    ov_status_t st = ov_compatibility_evaluate_os(hw,
                                                  pipeline_req->target_os ? pipeline_req->target_os : "macOS 12 Monterey",
                                                  &os_eval);
    if (st == OV_SUCCESS) {
        out_decision->compat_category = os_eval.category;
        strncpy(out_decision->compat_category_str, ov_compat_category_to_string(os_eval.category),
                sizeof(out_decision->compat_category_str) - 1);
    }

    /* 2. Retrieve Performance Profile */
    ov_perf_profile_config_t perf_conf;
    st = ov_perf_profile_get_by_id(pipeline_req->perf_profile_id, &perf_conf);
    if (st != OV_SUCCESS) {
        ov_perf_profile_get_by_id(OV_PERF_PROFILE_BALANCED, &perf_conf);
    }

    /* 3. Evaluate Workload with Topology and Performance preference */
    ov_integrated_request_t req = pipeline_req->workload;
    if (perf_conf.max_texture_clamp > 0 && (req.max_texture_dimension == 0 || req.max_texture_dimension > perf_conf.max_texture_clamp)) {
        req.max_texture_dimension = perf_conf.max_texture_clamp;
    }

    ov_resolution_result_t res;
    st = ov_resolver_evaluate_with_topology(&req, &hw->gpu_topology, &res);
    if (st == OV_SUCCESS) {
        out_decision->workload_resolution = res;
        out_decision->selected_gpu_index = res.selected_gpu_index;
        strncpy(out_decision->selected_gpu_name, res.selected_gpu_name, sizeof(out_decision->selected_gpu_name) - 1);
    }

    /* 4. Determine Backend Configuration */
    strncpy(out_decision->selected_backend, perf_conf.backend_preference, sizeof(out_decision->selected_backend) - 1);

    /* 5. Determine Boot Arguments & EFI Drivers */
    if (os_eval.requires_legacy_gpu_patch) {
        snprintf(out_decision->boot_arguments, sizeof(out_decision->boot_arguments),
                 "-v keepsyms=1 debug=0x100 ipc_control_port_options=0 amfi_get_out_of_my_way=1");
        snprintf(out_decision->efi_drivers_required, sizeof(out_decision->efi_drivers_required),
                 "OpenCore.efi, OpenRuntime.efi, AppleIntelHD4000Graphics.kext, GeForceKepler.kext");
    } else {
        snprintf(out_decision->boot_arguments, sizeof(out_decision->boot_arguments),
                 "-v keepsyms=1");
        snprintf(out_decision->efi_drivers_required, sizeof(out_decision->efi_drivers_required),
                 "OpenVintageBootApp.efi");
    }

    /* 6. Overall Pipeline Summary */
    snprintf(out_decision->overall_pipeline_summary, sizeof(out_decision->overall_pipeline_summary),
             "Hardware: %s [%s] | Target OS: %s (%s) | Profile: %s | Active GPU: %s (Index %u) | Routing: %s",
             hw->model_identifier,
             out_decision->is_real_hardware ? "REAL HARDWARE" : "SIMULATED HARDWARE",
             pipeline_req->target_os ? pipeline_req->target_os : "macOS",
             out_decision->compat_category_str,
             perf_conf.name,
             out_decision->selected_gpu_name,
             out_decision->selected_gpu_index,
             out_decision->workload_resolution.routing_path);
