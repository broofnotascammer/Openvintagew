...
ov_status_t ov_hardware_set_active_profile(ov_hw_profile_id_t profile_id) {
    ...
    if (active_profile.gpu_topology.gpu_count == 0) {
        if (active_profile.has_discrete_gpu) {
            ...
            active_profile.gpu_topology.active_gpu_index = 0; /* Default to primary/integrated GPU */
            ...
            active_profile.gpu_topology.active_gpu_index = 0;
        ...
        /* Preserve or set default active_gpu_index */
        if (active_profile.gpu_topology.active_gpu_index >= active_profile.gpu_topology.gpu_count) {
            active_profile.gpu_topology.active_gpu_index = 0;
        }
    }
...
const ov_gpu_info_t* ov_hardware_get_gpu(void) {
    if (active_profile.gpu_topology.gpu_count > 0 &&
        active_profile.gpu_topology.active_gpu_index < active_profile.gpu_topology.gpu_count) {
        return &active_profile.gpu_topology.gpus[active_profile.gpu_topology.active_gpu_index];
    }
...
uint32_t ov_hardware_get_active_gpu_index(void) {
    return active_profile.gpu_topology.active_gpu_index;
}

ov_status_t ov_hardware_set_active_gpu_index(uint32_t index) {
    if (active_profile.gpu_topology.gpu_count == 0) {
        if (index > 0) return OV_ERROR_INVALID_PARAM;
        active_profile.gpu_topology.active_gpu_index = 0;
        return OV_SUCCESS;
    }
    if (index >= active_profile.gpu_topology.gpu_count) {
        return OV_ERROR_INVALID_PARAM;
    }
    active_profile.gpu_topology.active_gpu_index = index;
    ov_log_info("Active GPU switched to index %u: %s (Physical inventory remains %u GPUs intact)",
                index, active_profile.gpu_topology.gpus[index].model_name,
                active_profile.gpu_topology.gpu_count);
    return OV_SUCCESS;
}

const ov_gpu_info_t* ov_hardware_get_active_gpu(void) {
    return ov_hardware_get_gpu();
}

const ov_gpu_info_t* ov_hardware_get_integrated_gpu(void) {
    if (active_profile.gpu_topology.has_integrated_gpu &&
        active_profile.gpu_topology.primary_gpu_index < active_profile.gpu_topology.gpu_count) {
        return &active_profile.gpu_topology.gpus[active_profile.gpu_topology.primary_gpu_index];
    }
    return &active_profile.gpu;
}

const ov_gpu_info_t* ov_hardware_get_discrete_gpu(void) {
    if (active_profile.gpu_topology.has_discrete_gpu &&
        active_profile.gpu_topology.discrete_gpu_index < active_profile.gpu_topology.gpu_count) {
        return &active_profile.gpu_topology.gpus[active_profile.gpu_topology.discrete_gpu_index];
    }
    return NULL;
}
